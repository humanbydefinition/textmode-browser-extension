...
Build and validate the Firefox MV3 package:

```sh
npm run lint:firefox
```

The Firefox zip generated by `npm run zip:firefox` is unsigned and is intended for AMO upload or temporary
debug installation. Normal installation in release Firefox requires a Mozilla-signed XPI. To create one for
self-distribution:

1. Create AMO API credentials at <https://addons.mozilla.org/en-US/developers/addon/api/key/>.
2. Copy `.env.example` to `.env`.
3. Set `WEB_EXT_API_KEY` to the JWT issuer value and `WEB_EXT_API_SECRET` to the JWT secret value.
4. Run:
...
The signed XPI is written to `.output/firefox-signed`.
